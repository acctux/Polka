...
def get_archive_items(archive):
    """List items in an archive using 'ouch'."""
    result = subprocess.run(
        ["ouch", "list", archive], capture_output=True, text=True, check=True
    )
...
def select_items(items):
    """Wrapper to launch curses UI and return selected items."""
...
def decompress_to_tmpfs(archive, selected_items):
    """Decompress archive into a temporary directory and prune unselected files."""
...
def move_to_destination(temp_dir, dest):
    """Move decompressed files to the destination directory."""
    dest = Path(dest)
    for path in temp_dir.rglob("*"):
        if path.is_file():
