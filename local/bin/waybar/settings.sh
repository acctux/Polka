...
# --- Now Playing ---
music_json=$("$HOME/Polka/local/bin/waybar/now-playing.sh" 2>/dev/null || echo '{}')
music_text=$(echo "$music_json" | jq -r '.text // empty' 2>/dev/null)

# --- Combine ---
if [[ -n "$music_text" ]]; then
  text="$music_text $notif_icon"
  alt="$music_text"
  main_class="music"
else
  text="$notif_icon"
...
jq -nc --arg text "$text" --arg alt "$alt" --arg tooltip "$tooltip" --arg class "$main_class" \
